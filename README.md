...
The **build** action is straightforward with the only real "trick" being an
option to use Xvfb. An X server is necessary for OpenSCAD to capture images.
So, roughly, the **build** action is:

  - *Checkout source* →
  - *Install OpenSCAD, optionally Xvfb, and any extra packages* →
  - *Optionally, run "\<command\> local-libraries" to set up model libraries* →
  - *Build with "\<command\> \<target\>" optionally under Xvfb (xvfb-run)*

The parameters of the **build** action are:

| Name                      | Type    | Default            | Description                                            |
| ------------------------- | ------- | ------------------ | ------------------------------------------------------ |
| target                    | string  | 'all'              | Build target(s) supplied to the command                |
...
| check\_target             | boolean | false              | Check if the _target_ string is empty and fail if so   |
...
moved into a separate subdirectory. So the **update-media-artifacts** action is:
  ...
  - *Remove any additional artifacts (optional)* →
...
| class\_name                 | string  | 'model'                       | Class attribute in \<div class="model"\> tag to search for  |
...
Combine two actions to update media icons, images, and STL. This uses the
...
release (using the action [softprops/action-gh-release](https://github.com/softprops/action-gh-release))
